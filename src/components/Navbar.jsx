...
const Navbar = () => {
  ...
  return (
    <nav
      className={`${styles.paddingX} w-full flex items-center py-5 fixed top-09 z-20 bg-primary`}
    >
      <div className="w-full flex justify-between items-center mx-w-7x1 mx-auto">
        ...
        >
          ...
          <p className="text-white text-[18px] font-bolt cursor-pointer">
            Samarpan sarkar<span className="sm:block hidden"> |My portfolio</span>
          </p>
        </Link>

        <ul className="listNone hidden sm:flex flex-row gap-10">
          {navLinks.map((link) => (
            <li
              key={link.id}
              className={`${
